...
export const Header = () => {
  ...
  const langLabels: Record<string, string> = { en: 'EN', ua: 'UA', ru: 'RU' };
  const currentLangIndex = languages.indexOf(lang);
  const nextLang = languages[(currentLangIndex + 1) % languages.length];

  const cycleLanguage = () => setLanguage(nextLang);

  return (
    <header className="fixed top-0 left-0 w-full z-[1000] bg-white/90 dark:bg-slate-900/95 border-b border-slate-200 dark:border-slate-700">
      <div className="max-w-[1200px] mx-auto px-6 py-4 flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="hidden lg:flex w-11 h-11 rounded-full items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-semibold text-sm transition-all"
      ...
      {isOpen && (
        <>
          <div
            className="fixed inset-0 z-[999] lg:hidden bg-black/40"
            onClick={() => setIsOpen(false)}
          />
          <BurgerMenu isDark={isDark} toggleTheme={toggleTheme} onClose={() => setIsOpen(false)} isOpen={isOpen} />
        </>
      )}
    </header>
  );
